...
class Trading():
    ...
    # Define static vars
    WAIT_TIME_BUY_SELL = 0.5 # seconds
    WAIT_TIME_CHECK_BUY = 0.3 # seconds
    WAIT_TIME_CHECK_SELL = 2 # seconds
    ...
    def sell(self, symbol, quantity, orderId, sell_price, last_price):
        ...
        if buy_order['status'] == 'FILLED' and buy_order['side'] == "BUY":
            ...
            if buy_order['status'] == 'FILLED' and buy_order['side'] == "BUY":
                print ("Bought after 0.5 sec... Try sell...")
                ...
                print ("Quantity to sell = %d") % (quantity)
        ...
        if self.stop_loss > 0:
            ...
            sell_status = 'NEW'
            stopcycle = False
            while (sell_status != "FILLED" or stopcycle == True):
                ...
                profitloss = (sell_price - lastprice) *100 / sell_price
                print ('Status: %s Current price: %s Sell price: %s Profit losing: %.4s' % (sell_status, lastprice, sell_price, profitloss))
                lossprice = sell_price - (sell_price * self.stop_loss / 100)
                if lastprice < lossprice: 
					print ('Not sold by good price, wait 2 more seconds...')
					time.sleep(self.WAIT_TIME_CHECK_SELL)
        ...
        if self.max_amount:
			...
			print ('Amount to buy in BTC: %s' % quantity)       
        self.order_id = 0
        self.bot_status = "sell"
        ...
        while (partial_status == "hold"):
            ...
            if order['status'] == 'PARTIALLY_FILLED':
                ...
                if self.min_notional > quantity * sell_price:
                    ...
                    print ("Checking partiall order, id: %d" % orderId)
                    self.cancel(symbol, orderId) #Почему то не словил ордер номер здесь, и тупо оставил "сдачу" продолжил торговать
                    partial_status = "sell"
    ...
    def action(self, symbol):
        ...
        if self.option.prints and self.order_id == 0:
         ...
         print ('  Price:%.8f Bid:%.8f Ask:%.8f Spread:%.2f' % (lastPrice, lastBid, lastAsk, spreadPerc))

        '''
        Если lastAsk >= lastbid + profit тогда покупаем
        '''
    ...
    def run(self):
        ...
        print ('Start trading on Binance.com. --Pair:%s\n' % symbol)
